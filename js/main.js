...
let sel = null;          // 選択中 {kind:'piece',x,y,moves} | {kind:'hand',id,drops}
const pieceEls = new Map(); // uid -> DOM要素
...
function enterTitle() {
  ...
  $('title-boss-l').src = boss.img;
  $('title-boss-r').src = YOKAI[ENEMY_BOSS].img;
...
function cellEl(x, y) { return $('board-cells').children[y * COLS + x]; }
...
function renderAll() {
  ...
  for (let y = 0; y < ROWS; y++) {
    for (let x = 0; x < COLS; x++) {
      ...
      if (!pc) continue;
      seen.add(pc.uid);
      let el = pieceEls.get(pc.uid) || makePieceEl(pc);
...
function renderHand(side) {
  ...
  for (const id in G.hands[side]) {
    ...
    chip.innerHTML = `<img src="${YOKAI[id].img}" alt="${YOKAI[id].name}" draggable="false">` +
...
function onCellClick(x, y) {
  ...
  if (sel) {
    if (sel.kind === 'piece') {
      ...
      if (m) { doAction({ kind: 'move', from: { x: sel.x, y: sel.y }, to: { x, y } }); return; }
    } else if (sel.kind === 'hand') {
      const d = sel.drops.find(d => d.x === x && d.y === y);
      if (d) { doAction({ kind: 'drop', id: sel.id, to: { x, y } }); return; }
    }
  }
...
function startBattle() {
  ...
  const boss = YOKAI[Meta.bossId()];
  $('player-avatar').src = boss.img;
  $('player-name').textContent = boss.name;
  $('enemy-avatar').src = YOKAI[ENEMY_BOSS].img;
  renderAll();
  updateHUD();
...
function showResult() {
  ...
  $('result-boss').src = YOKAI[win ? Meta.bossId() : ENEMY_BOSS].img;
  if (win) {
  ...
  $('result-sub').textContent = reasons[G.reason] || '';
