...
百鬼盤を「百鬼盤」などの検索語で見つけてもらうための、公開後に運営者が行う作業手順。

## 目的

- Googleに `https://yokai-shogi.nit-games.com/` を正式URLとして認識させる。
- 検索結果のタイトルと説明文に「百鬼盤」「妖怪を集めて、取って、HPを削る対戦ゲーム」が出やすい状態にする。
...
5. 外部リンクを作る。
   ...
   - `nit-games.com` 側にゲーム一覧ページがある場合、百鬼盤へのリンクを追加する。

6. 検索状況を確認する。
   - 数日から数週間後に `site:yokai-shogi.nit-games.com` でGoogle検索する。
   - Search Consoleの「検索パフォーマンス」で、クエリ `百鬼盤` の表示回数とクリック数を見る。

## 注意点

- 検索順位は即時には変わらない。反映には数日から数週間、場合によっては数か月かかる。
- 「百鬼盤」で上位表示したい場合、外部から自然な文脈で `百鬼盤` とリンクされることが重要。
