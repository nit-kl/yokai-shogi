...
ストアページ編集の「基本情報」タブ用。空欄は意図的。Steam の対応言語は日本語のみ（英語 UI をストアでは謳わない）。
...
| アプリのタイプ | ゲーム |
| ゲーム名 | `百鬼盤` |
| 英語名（言語別） | **付けない**。対応言語に英語を出さないため、`Hyakkiban` はストアに載せない。Steam エディタの English ドロップダウンはフォールバック用で、中身は日本語の `百鬼盤` のままでよい（[言語ドキュメント](https://partner.steamgames.com/doc/store/localization/languages)） |
...
音声・字幕付きの台詞はない。ストアの「対応言語」は日本語のみ。英語をオンにすると、英語タイトルと英語 Capsule が必須になる。
...
中国語・韓国語は未翻訳なので付けない。審査で「未翻訳なのに対応」と見なされる。クライアントの英語オーバーレイは残してよいが、Steam では英語対応と書かない。

English は Steam のフォールバック言語なので、ストア編集の言語ドロップダウン「English」側にも説明文・Capsule を入れる。中身は日本語（`百鬼盤` と日本語 Capsule）でよい。英語ロゴ版は上げない。
