...
const STATIC: Record<string, string> = {
  ...
  '対戦相手を探しています。15秒ほどでAI対戦に切り替わります':
    'Searching for an opponent. An AI battle starts in about 15 seconds',
  '対戦相手が見つからないため、AI対戦を準備しています…': 'No opponent yet. Preparing an AI battle…',
  'AI対戦を準備しています…': 'Preparing an AI battle…',
  ...
  '挑戦': 'Play', 'ヒント': 'Hint',
  '報酬を確認中…': 'Checking rewards…',
  '課題の報酬を受け取れませんでした': 'Could not claim the puzzle reward.',
  'この課題のチケットは受取済みです': 'This puzzle’s ticket has already been claimed.',
  'チケット所持上限のため付与できませんでした': 'Could not grant a ticket because you are at the cap.',
  '課題の報酬を受け取れませんでした(通信状態を確認)': 'Could not claim the puzzle reward. Check your connection.',
  '手数内に課題を達成できなかった… ヒントを見て再挑戦しよう': 'The puzzle was not cleared in time. Check the hint and try again.',
  '前へ進め': 'Step Forward', '斜めに跳べ': 'Strike Diagonally', '遠くから斬れ': 'Cut From Afar',
  '成りの力': 'Power of Promotion', '持ち駒を打て': 'Drop Into Play', '連続で取れ': 'Chain Captures',
  '敵陣で強襲': 'Assault the Camp', '駒を飛び越えろ': 'Leap Over', '天狗の羽ばたき': 'Tengu Wings',
  '敵陣で成れ': 'Promote Then Break',
  '1手で敵大将を討て': 'Capture the enemy general in one move',
  '斜めの動きで大将を討て': 'Capture the general with a diagonal step',
  '長い移動で大将を討て': 'Capture the general with a long slide',
  '成駒を使って大将を討て': 'Capture the general with a promoted piece',
  '持ち駒を打ち、次の手で大将を討て': 'Drop a piece, then capture the general',
  '2連続で取り、コンボを決めろ': 'Capture twice in a row to score a combo',
  '敵陣の取りで魂力を0にしろ': 'Capture in the enemy camp to drain HP to 0',
  '跳びの動きで大将を討て': 'Jump over a piece to capture the general',
  '斜めに飛んで大将を討て': 'Fly diagonally to capture the general',
  '敵陣で成り、成駒で大将を討て': 'Promote in the enemy camp, then capture the general',
...
const YOKAI_EN: Record<string, YokaiEnglish> = {
  kyubi: ['Nine-Tailed Fox', '1 square in any direction', 'Lord of the Moon', 'On a full moon, allied captures deal 2× damage', 'Nine-Tails Unsealed'],
  shuten: ['Shuten-doji', '1 square in any direction', 'Sake Cellar', 'Restores 50 HP at the end of your turn. Your Hunger grace is +4 moves', 'Oni God Rakshasa'],
  ...
  nurarihyon: ['Nurarihyon', '1 square in any direction', 'Commander of the Night Parade', 'Allied captures deal +5% damage per other ally (up to +40%)', 'Supreme Night Parade'],
...
const resonanceEn = [
  ['Oni Feast', 'While both are on the board, Sake Cellar healing is +25'],
    ['Foxfire Bond', "When one is captured, the other's next capture is a guaranteed critical"],
] as const;
...
function translatePatterns(value: string): string {
  ...
  const replacements: Array<[RegExp, string]> = [
    ...
    [/第(\d+)問/g, 'Puzzle $1'], [/残り(\d+)手/g, '$1 moves left'],
    [/(.+) を達成した!/g, 'Cleared $1!'],
    [/クリア報酬:\s*ガチャチケット\s*🎟\s*\+(\d+)/g, 'Clear reward: Ticket 🎟 +$1'],
    ...
    [/対戦相手を探しています… 約(\d+)秒でAI対戦に切り替わります/g,
      'Searching for an opponent… AI battle in about $1 seconds'],
    ...
    [/異装:\s*性能は通常版と同じ/g, 'Alt: Same stats as the standard version'],
    [/月の主 満月の妖気 ×([\d.]+)!/g, 'Moon Lord · Full-moon aura ×$1!'],
    [/骨の山 \+(\d+)%!/g, 'Mountain of Bones +$1%!'],
    [/骨が積まれた\((\d+)\)/g, 'A bone was piled ($1)'],
    [/(.+)を味方にした!/g, '$1 joined your army!'],
    [/首が逃げた! 残り(\d+)首/g, 'A head slithered away! $1 heads left'],
    [/覚醒:\s*(.+)（自ターン3回 ATK×([\d.]+)）/g, 'Awakening: $1 (ATK ×$2 for three of your turns)'],
    [/因縁【(.+)】/g, 'Resonance [$1] '],
    [/【成】/g, ' [Promoted]'], [/反撃ダメージ\s*(\d+)!/g, 'Counter damage: $1!'],
