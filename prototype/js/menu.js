...
const MenuUI = {
  ...
  onEnterTitle() {
    ...
    if (bonus) {
      $('login-day').textContent = bonus.day;
  ...
  refreshCurrency() {
    ...
    $('btn-pull1').disabled = d.tickets < 1;
    $('btn-pull10').disabled = d.tickets < 10;
    $('btn-exchange').disabled = d.yoryoku < Meta.EXCHANGE_COST;
  },
  ...
  placedIds() { return new Set(this.rows.flat().filter(Boolean)); },
    ...
    for (let ry = 0; ry < 2; ry++) {
      for (let x = 0; x < COLS; x++) {
        ...
        if (id) {
          ...
          cell.innerHTML = `<img src="${def.img}" alt="${def.name}" draggable="false">`;
        }
