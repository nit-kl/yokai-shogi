/* ガチャ・編成UIの検証(旧 prototype/test/ui-gacha.js の移植)
...
await page.screenshot({ path: path.join(dir, 'shot-g1-login.png') });
await page.click('#btn-login-ok');
await page.waitForTimeout(400);
await page.screenshot({ path: path.join(dir, 'shot-g2-title.png') });

/* ガチャ画面 → 10連(初回10枚+ログボ1枚=11枚あるはず) */
await page.click('#btn-gacha');
await page.waitForSelector('#screen-gacha.active');
await page.waitForTimeout(400);
await page.screenshot({ path: path.join(dir, 'shot-g3-gacha.png') });

await page.click('#btn-pull10');
await page.waitForSelector('#gacha-result:not(.hidden)');
await page.waitForTimeout(1800);
await page.screenshot({ path: path.join(dir, 'shot-g4-pull10.png') });
...
await page.screenshot({ path: path.join(dir, 'shot-g5-pull1.png') });
await page.click('#btn-gacha-ok');
const disabled = await page.evaluate(() =>
  window.yk.$('btn-pull1').disabled && window.yk.$('btn-pull10').disabled);
...
await page.evaluate(() => {
  const { yk } = window;
  for (const id of yk.GACHA_POOL) yk.Meta.data.owned[id] = 1;
  yk.Meta.save();
});
await page.click('#btn-formation');
await page.waitForSelector('#screen-formation.active');
await page.waitForTimeout(400);
await page.screenshot({ path: path.join(dir, 'shot-g6-formation.png') });
...
await page.screenshot({ path: path.join(dir, 'shot-g7-form-edited.png') });
await page.click('#btn-form-back');
await page.waitForSelector('#screen-title.active');
await page.waitForTimeout(600);
await page.screenshot({ path: path.join(dir, 'shot-g8-title-tamamo.png') });
...
await page.screenshot({ path: path.join(dir, 'shot-g9-battle.png') });
